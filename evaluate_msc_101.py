...
from deeplab_resnet import DeepLabResNetModel_101, ImageReader, prepare_label

IMG_MEAN = np.array((104.00698793, 116.66876762, 122.67891434), dtype=np.float32)

RESTORE_FROM = './DeepLab2/'
...
def main():
    ...
    with tf.variable_scope('', reuse=False):
        net = DeepLabResNetModel_101({'data': image_batch}, is_training=False, num_classes=args.num_classes)
    with tf.variable_scope('', reuse=True):
        net075 = DeepLabResNetModel_101({'data': image_batch075}, is_training=False, num_classes=args.num_classes)
    with tf.variable_scope('', reuse=True):
        net05 = DeepLabResNetModel_101({'data': image_batch05}, is_training=False, num_classes=args.num_classes)
