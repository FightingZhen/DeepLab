...
from deeplab_resnet import DeepLabResNetModel_34, ImageReader, decode_labels, inv_preprocess, prepare_label

IMG_MEAN = np.array((104.00698793, 116.66876762, 122.67891434), dtype=np.float32)

BATCH_SIZE = 4
SNAPSHOT_DIR = './DeepLab_34_tuned/'
DATA_DIRECTORY = './Data/Eye_picture/Training/'
DATA_LIST_PATH = './Data/Eye_picture/train.txt'
SAVE_NUM_IMAGES = 4
SAVE_PRED_EVERY = 204
GRAD_UPDATE_EVERY = 2
IGNORE_LABEL = 255
...
LEARNING_RATE = 1e-5
NUM_CLASSES = 2
NUM_STEPS = SAVE_PRED_EVERY // BATCH_SIZE * 300 + 1
POWER = 0.9
RANDOM_SEED = 1234
RESTORE_FROM = None
WEIGHT_DECAY = 1e-4
...
def main():
    ...
    with tf.variable_scope('', reuse=False):
        net = DeepLabResNetModel_34({'data': image_batch}, is_training=args.is_training, num_classes=args.num_classes)
    with tf.variable_scope('', reuse=True):
        net075 = DeepLabResNetModel_34({'data': image_batch075}, is_training=args.is_training,
                                       num_classes=args.num_classes)
    with tf.variable_scope('', reuse=True):
        net05 = DeepLabResNetModel_34({'data': image_batch05}, is_training=args.is_training,
                                      num_classes=args.num_classes)
    ...
    label_proc075 = prepare_label(label_batch, tf.stack(raw_output075.get_shape()[1:3]), num_classes=args.num_classes,
                                  one_hot=False)
    label_proc05 = prepare_label(label_batch, tf.stack(raw_output05.get_shape()[1:3]), num_classes=args.num_classes,
                                 one_hot=False)
    ...
    reduced_loss = tf.reduce_mean(loss) + tf.reduce_mean(loss100) + tf.reduce_mean(loss075) + tf.reduce_mean(
    ...
    tf.summary.image('images', tf.concat(axis=2, values=[images_summary, labels_summary, preds_summary]),
    ...
    for step in range(args.num_steps):
        ...
        if step % args.save_pred_every == 0:
            images, labels, summary, _ = sess.run([image_batch, label_batch, merged, train_op], feed_dict=feed_dict)
